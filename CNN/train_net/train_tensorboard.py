...
try:
    ...
    print('importing mobilenet: error')
'''
from net.mobilenet_v2.mobilenet_v2 import mobilenet,training_scope

def arch_mobilenet_v2(X, num_classes, dropout_keep_prob=0.8, is_train=False):
    arg_scope = training_scope()
    with slim.arg_scope(arg_scope):
        net, end_points = mobilenet(X, is_training=is_train)
    with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], stride=1, padding='SAME'):
        with tf.variable_scope('Logits_out'):
            net = slim.conv2d(net, 200, [1, 1], activation_fn=None, normalizer_fn=None, scope='Logits_out0')
            net = slim.dropout(net, dropout_keep_prob, scope='Dropout_1b_out0')
            net = slim.conv2d(net, 100, [1, 1], activation_fn=None, normalizer_fn=None, scope='Logits_out1')
...
def train(train_data,train_label,valid_data,valid_label,train_n,valid_n,IMAGE_HEIGHT,IMAGE_WIDTH,learning_rate,num_classes,epoch,batch_size=64,keep_prob=0.8,
    ...
    elif arch_model == "arch_mobilenet_v2":
    ...
    gs=tf.Variable(0)
    lr = tf.train.exponential_decay(learning_rate=learning_rate, global_step=gs,decay_steps=2000,decay_rate=0.9)
    with tf.control_dependencies(update_ops):
        optimizer = tf.train.MomentumOptimizer(learning_rate=lr,momentum=0.9).minimize(loss, var_list=var_list,global_step=gs)
